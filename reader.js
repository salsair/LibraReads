...
class BookReader {
    ...
    setupEventListeners() {
        ...
        // Click outside settings to close (UPDATED to use toggle function)
    ...
    async loadBookContent() {
        try {
            ...
            if (!this.bookContent) {
                throw new Error('Book content is not available.');
            }
            ...
            this.showError(error.message || 'Failed to load book content.');
        }
    }
    ...
    async generatePages() {
        return new Promise((resolve) => {
            ...
            if (this.pages.length === 0) {
                this.pages.push('<p>Content not available.</p>');
            }
    ...
    renderCurrentPage() {
        ...
        const content = this.pages[this.currentPage - 1] || '<p>Page not found.</p>';
    ...
    /**
     * Toggle settings panel (UPDATED)
     */
    toggleSettings() {
        const isActive = this.elements.settingsPanel?.classList.toggle('active');
        document.body.classList.toggle('settings-open', isActive);
    }
    ...
    handleKeyboard(e) {
        ...
        switch(e.key) {
            ...
            case 'ArrowRight':
                ...
                break;
            case 'Escape':
                // UPDATED: Use toggleSettings for consistency
                if (this.elements.settingsPanel?.classList.contains('active')) {
                    this.toggleSettings();
    ...
    loadSettings() {
        try {
            ...
            if (settings.fontSize) {
                this.fontSize = Number(settings.fontSize);
                if (this.elements.bookContent) {
                    this.elements.bookContent.style.fontSize = `${this.fontSize}px`;
                }
                if (this.elements.fontSizeDisplay) {
            ...
            if (settings.contentWidth) {
                this.contentWidth = Number(settings.contentWidth);
